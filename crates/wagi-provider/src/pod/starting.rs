...
impl State<PodState> for Starting {
    ...
    async fn next(
        ...
        if let Some(default_host_annotation) =
            pod.annotations().get(WAGI_ANNOTATION_KEY_DEFAULT_HOST)
        {
                ...
                Ok(modules) => {
                    let modules_map: indexmap::IndexMap<String, WagiModuleConfig> = pod
                        ...
                        .fold(modules, |mut modules, (name, image)| {
                            modules
                                .entry(name)
                                .and_modify(|m| m.module = format!("oci://{}", image));
                            modules
                        });

                    wagi_modules_config.modules = modules_map.into_iter().map(|(_, m)| m).collect();
                }
                Err(parse_err) => {
                    return Transition::next(
                        ...
                        Error::new(format!(
                            ...
                            WAGI_ANNOTATION_KEY_MODULES, parse_err,
                        )),
                    );
                }
            }
        ...
        tokio::task::spawn(async move {
            ...
            match wagi_modules_config.build_wagi_router().await {
                ...
                Err(error) => tx.send(Err(anyhow::anyhow!("{}", error))).await,
            }
        });
